    ...
    <div className="p-6 md:p-12 max-w-6xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6 border-zinc-100 dark:border-zinc-800">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Discover and book the best events around you.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:max-w-xl">
          ...
          <Link href="/my-bookings" className="w-full sm:w-auto">
            <Button variant="outline" className="w-full border-zinc-200 shadow-2xs dark:border-zinc-800">
              View My Bookings
            </Button>
          </Link>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-10">
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start gap-2 mb-2">
                    ...
                    <Badge
                      variant={event.availableTickets > 0 ? "secondary" : "destructive"}
                ...
                <CardContent className="pb-4">
                  <div className="space-y-2 text-sm border-t border-dashed border-zinc-100 dark:border-zinc-800 pt-3">
                    <div className="flex justify-between items-center">
                      <span className="text-muted-foreground flex items-center gap-1.5"><Coins className="w-3.5 h-3.5 text-zinc-400" /> Price</span>
                      <span className="font-bold text-emerald-600 dark:text-emerald-400 text-base">
                        {event.ticketPrice.toLocaleString()} MMK
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-muted-foreground flex items-center gap-1.5"><Ticket className="w-3.5 h-3.5 text-zinc-400" /> Capacity</span>
                ...
                <CardFooter className="pt-0">
                  <Link
                    href={`/booking/${event.id}?name=${encodeURIComponent(event.title)}&price=${event.ticketPrice}`}
                    className="w-full"
                  >
                    <Button
                    ...
                    >
                      {event.availableTickets > 0 ? (
                        <span className="flex items-center gap-1">Buy Tickets <ArrowRight className="w-3.5 h-3.5" /></span>
                      ) : (
                        "Sold Out"
                      )}
                    </Button>
          ...
          {/* ✅ ၅။ နောက်ထပ် Event ကျန်သေးရင်မှ ပေါ်လာမည့် Load More ခလုတ် */}
          {filteredEvents.length > visibleCount && (
            <div className="flex justify-center pt-2">
              ...
              >
                Load More Events <ChevronDown className="w-4 h-4 animate-bounce duration-1000" />
              </Button>
            </div>
          )}
        </div>
