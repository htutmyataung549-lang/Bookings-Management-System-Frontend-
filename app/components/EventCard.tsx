...
// import { CalendarDays, Coins, Ticket, ArrowRight } from "lucide-react";
...
//             }`}
//           >
//             {isAvailable ? `Available: ${event.availableTickets}` : "Sold Out"}
//           </Badge>
//         </div>
...
//               isAvailable ? "bg-emerald-600 hover:bg-emerald-700 text-white" : ""
//             }`}
    ...
    <Card className="flex flex-col justify-between overflow-hidden border-zinc-200/80 dark:border-zinc-800 shadow-2xs hover:shadow-md transition-all duration-200 bg-card text-card-foreground group">
      ...
      <CardFooter className="pt-0">
        ...
        >
          ...
          >
            {isAvailable ? (
              <span className="flex items-center gap-1">Buy Tickets <ArrowRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5" /></span>
            ) : (
              "Sold Out"
            )}
          </Button>
